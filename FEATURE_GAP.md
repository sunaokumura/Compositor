# FEATURE_GAP — Mac版 vs Windows移植版（t_f04eebff時点に更新）
...
- 詳細は TOOLS_PARITY.md の×17件を参照（ContentFill/SubjectRemoval/Distort/Curves/Levels/GradientMap 等はいずれも無効ボタン＋ツールチップ）。
- レイヤーマスク・フォルダ/グループ・キャンバスサイズ変更・Crop・Shape・プロジェクト保存・シート・数値Exact入力・Flip Canvasは次フェーズ。

## 実用レベル6項目の自己評価（t_f04eebff更新）
1. 機能網羅: TOOLS_PARITY.mdで41件を○8/△16/×17に整理。×は全て未対応バッジ明示
2. 安定性: クラッシュ修正済み（t_3b5fced0）・単体42件合格（描画系新規11件含む）・1000回ストレス合格（既存）。100時間相当の長時間実機は安定化カードの範囲
